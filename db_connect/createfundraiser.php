...
if(!isset($_POST['submit'])) { //if one try to access createfundraiser.php without access
    header("Location: ../createfundraiser.php");
    exit();
} else {
    include_once 'dbh.php'; //creating connection to database
    ...
    if(!preg_match("/^[a-zA-Z'. -]+$/", $campaignName)) {    
        header("Location: ../createfundraiser.php?campaign=invalidname");
        exit();
    } else {
        ...
        if($resultCheck > 0) {
            header("Location: ../other/fundraiserFail.html?campaign=camapignNameTaken");
            exit();
        } else {
            ...
            $campaignImagePath = "images/fundraisers/".$filename;
...
// to-do
             ...
             if($insertSuccess) {
                ...
                header("Location: ../other/fundraiserSuccess.html");
             } else {
                 echo $conn->error;
             }
        }
