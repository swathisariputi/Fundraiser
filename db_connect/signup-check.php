...
if(!isset($_POST['submit'])) { //if one try to access signup-check.php without access
    ...
    if(!preg_match("/^[a-zA-Z'. -]+$/", $fullname)) {    
        ...
        if($resultCheck > 0) {
            ...
            if($insertSuccess) {
                // send mail
                $url = "http://localhost/IWP_WEB/db_connect/verifyEmail.php?vkey=".$vkey;
                $to = $email;
                $subject = 'Email Verification';
                $message = '<p>You can verify the email from the link below:</br>';
                $message .= '<a href="' .$url. '">Verify Email</a></p>';

                $headers = "From: Fund Raiser <iwpassignment2021@gmail.com>\r\n";
               // $headers .= "Reply-To: iwpassignment2021@gmail.com\r\n";
